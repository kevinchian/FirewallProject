            ...
            if (protocol == 6) and (external_port == 80) and (send == True) : #tcp, dest port == 8, send == True
                ...
                if (pkt_dir == PKT_DIR_OUTGOING):   #request
                    ...
                    if (not (TCPConnection in self.httpDict)) and (flagsTCP == 2):  #creating a new TCP connection for this http request
                        ...
                        if (seq > self.httpDict[TCPConnection][2]): #if seq is greater than what we expected (out of order packet)
                            send = False    #drop packet
                        elif (seq < self.httpDict[TCPConnection][2]): #if seq is less (retransmitted packet)
                            a=1
                            ...
                            self.httpDict[TCPConnection][2] = seq+httpData  #update next expected seq number
                    ...
                    if (flagsTCP == 18):   #is a syn packet to get initial seq for responseSeq.
                        ...
                        if (seq > self.httpDict[TCPConnection][3]): #if seq is greater than what we expected (out of order packet)
                            send = False    #drop packet
                        elif (seq < self.httpDict[TCPConnection][3]): #if seq is less (retransmitted packet)
                            #Let it be sent, don't do anything in http
                            a = 1
                        else:   #seq is what we expected
                            self.httpDict[TCPConnection][3] = seq+httpData  #update next expected seq number
